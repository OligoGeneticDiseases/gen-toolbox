...
if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(
            description=f"Gnomad frequency table burden analysis pipeline command-line "
            f"tool using Hail\n{hl.cite_hail()}"
        )
        ...
        if args.command is None:
            ...
            if str.lower(ch.args.command) == "findtype":  # Handle findtype command
                ...
                conf.set("spark.app.name", "HailTools-TSHC")
                conf.set("spark.executor.memory", "4g")
                conf.set("spark.driver.memory", "56g")
                conf.set(
                    "spark.jars", "{0}/backend/hail-all-spark.jar".format(hail_home)
                )
                conf.set("spark.executor.extraClassPath", "./hail-all-spark.jar")
                conf.set(
                    "spark.driver.extraClassPath",
                    "{0}/backend/hail-all-spark.jar".format(hail_home),
                )
                conf.set(
