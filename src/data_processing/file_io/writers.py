...
def write_filelist(
    dir, file_name, file_paths, include_duplicates=False, verbose=1, regex=None
):
    """
    ...
    try:
        ...
        with open(os.path.join(dir, file_name), "w") as f:
            ...
            for file_path_pair in file_paths:
                ...
                prefix = trim_prefix(
                    file_path_pair[0]
                )  # E0000000 /trimmed by "." and "_"
                # Is unique
                if prefix not in unique_prefixes:
                    if regex is not None:
                        if (
                            eval_regex(full_path, regex_rules) is not None
                        ):  # there is a match with the corresponding regex
                            unique_prefixes.append(
                                prefix
                            )  # it is unique and added to the list
                            f.write(
                                prefix + "\t" + full_path + "\n"
                    ...
                    duplicate_prefixes.append(
                        [prefix, file_path_pair[0], file_path_pair[1]]
                    )
            ...
            if verbose > 0:
                ...
                print(
                    "Regex matches {0}, regex filtered out {1} lines.".format(
                        len(unique_prefixes), regexed_out
                    )
                )
            if not include_duplicates:  # write the duplicates to a separate file
                ...
                if len(duplicate_prefixes) > 0:
                    if verbose > 0:
                        print(
                            "Creating duplicate sample list in {0}".format(
                                duplicate_filename
                            )
                        )
...
def copy_vcf(files, dest, overwrite=False):
    ...
    print(
        "Finished copying of {0} files. Renamed {1} files. ".format(
            len(unique_copied), len(renamed)
        )
    )
...
def write_filelist(
    input_dir, file_name, file_paths, include_duplicates=False, verbose=1, regex=None
):
    """
    This function writes lists of files that have been inputted as arrays.
